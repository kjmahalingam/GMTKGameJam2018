const instructionState = {
  ...
  create: () => {
    ...
    let mainInstructionText = game.add.text(game.world.width / 2, 70, instructionString, instructionStyle);
    ...
    let attackTowerInstruction = game.add.text(game.world.width / 2, 250, attackTowerString, instructionStyle);
    attackTowerInstruction.anchor.set(0.5, 0.5);
    
    attackTowerExample = game.add.sprite(game.world.width / 2, 180, "attackTower");
    attackTowerExample.anchor.setTo(0.5, 0.5);
    attackTowerExample.width = attackTowerExample.height = 40;
    
    
    detonateTowerString = "This is a detonation tower. When it's health expires, it deals 5 damage to all enemies in range. It costs 3 resources and can be added with the W key.";
    let detonateTowerInstruction = game.add.text(game.world.width / 2, 400, detonateTowerString, instructionStyle);
    detonateTowerInstruction.anchor.set(0.5, 0.5);
    
    detonateTowerExample = game.add.sprite(game.world.width / 2, 330, "detonateTower");
    detonateTowerExample.anchor.setTo(0.5, 0.5);
    detonateTowerExample.width = detonateTowerExample.height = 40;
    
    incomeTowerString = "This is an income tower. It increases the amount of resources you receive every interval by one. They cannot be built, but you will earn a new one with each enemy base you defeat.";
    let incomeTowerInstruction = game.add.text(game.world.width / 2, 550, incomeTowerString, instructionStyle);
    incomeTowerInstruction.anchor.set(0.5, 0.5);
    
    incomeTowerExample = game.add.sprite(game.world.width / 2, 480, "incomeTower");
